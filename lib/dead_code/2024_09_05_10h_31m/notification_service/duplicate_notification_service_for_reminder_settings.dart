...
class DuplicateNotificationServiceForReminderSettings {
  ...
  Future<Map<String, dynamic>> getUserNotificationSettings() async {
    ...
    if (user != null) {
      ...
      if (doc.exists) {
        log("User settings found: ${doc.data()}");
        return doc.data()!;
      }
    }
    log("User settings not found, using default values.");
    return {
      'reminderEnabled': false,
      'reminderTime': 10, // Default value
    };
  }
  ...
  Future<void> scheduleNotification({
    ...
    final userSettings = await getUserNotificationSettings();
